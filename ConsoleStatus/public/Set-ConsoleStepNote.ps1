function Set-ConsoleStepNote {
    <#
        .SYNOPSIS
            Attaches a note to the current item, always written on its own line underneath.

        .DESCRIPTION
            Where Set-ConsoleStepDetail competes for the space next to the status and is truncated
            into a note only when it does not fit, a note is always a note. It is the place for the
            full path, the full command line or the full error that the value column had to cut.

            Call it from inside an Invoke-ConsoleStep action, or between Write-ConsoleItem and
            Write-ConsoleResult.

        .PARAMETER Text
            The note. Any length: it is wrapped across as many lines as it needs. Whitespace is
            collapsed. An empty string clears a note set earlier in the same item.

        .PARAMETER Append
            Keep what is already there and add this behind it, separated by '; '. Without it the
            last call wins, so a step that reports several findings loses all but the last. An
            empty string appends nothing rather than clearing.

        .EXAMPLE
            Set-ConsoleStepNote -Text $fullPath
    ...
    $text = Format-SingleLine -Text $Text

    if (-not $Append) {
        $script:ConsoleStatus.Note = $text
        return
    }
    ...
    $existing = $script:ConsoleStatus.Note
    $script:ConsoleStatus.Note = if ([string]::IsNullOrEmpty($existing)) { $text } else { "$existing; $text" }
}

# SIG # Begin signature block
# MII6AgYJKoZIhvcNAQcCoII58zCCOe8CAQExDzANBglghkgBZQMEAgEFADB5Bgor
# BgEEAYI3AgEEoGswaTA0BgorBgEEAYI3AgEeMCYCAwEAAAQQH8w7YFlLCE63JNLG
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCAEAdeEZ7NWiKiO
# cirj9AvkRt1u4eiUvE2Y77DbuE2IYKCCIiYwggXMMIIDtKADAgECAhBUmNLR1FsZ
...
# ADAZBgkqhkiG9w0BCQMxDAYKKwYBBAGCNwIBBDAvBgkqhkiG9w0BCQQxIgQgW0tS
# MY2eHWPtKsv61olH9tT8o07m85GrTGV1TzXm5uEwDQYJKoZIhvcNAQEBBQAEggGA
# fJiwn2zAM1OkT8Dz87eE1+uh1PtzvtybAFENzCgqpnhqBmyeM9cBZUepeMyRJe+C
# UoBw8VGFA2Hr6n8RgJk/+piXeGISgRuePqCCv9HHIW+CDTn0ch675CTkggVDcpV8
# jlvnsSWdlC9dWrloNEDVomU16mEWbRWqbhjUbbSfBofjhtIXaheeyPVkn5eQLtgJ
# yC0d+CBolpjn1KcUdALhNI1jNXe0gqPUeA8csN0fo5Ipy89KU2u9zjcnNp7sXolJ
# vQFMiXCPhpO6zYp1EnfltoDzCPq7wv5MhffDsammWIQzaenSd2VcebEG7tQhkndw
# 3dUsj5pCeBPUBCSqA+Uzfk7WIRv41xeUBf+zjsMoS5J+i6AuHKRQHQfq2SEB5dGq
# 0l6hgCrjk6cbwYRbqpjowUZn6xwuxl6j1Tgd8LSNSvIzq7YXJ+EPF4gdY/4b568d
# AUrrYIc+RZyxr+psYD7Kms+TisxR7XXYz3zlimT9XPeQ5h4U2bc4E0/T11BOS8ke
# oYIUsjCCFK4GCisGAQQBgjcDAwExghSeMIIUmgYJKoZIhvcNAQcCoIIUizCCFIcC
# AQMxDzANBglghkgBZQMEAgEFADCCAWoGCyqGSIb3DQEJEAEEoIIBWQSCAVUwggFR
# AgEBBgorBgEEAYRZCgMBMDEwDQYJYIZIAWUDBAIBBQAEILbfI86vyaAOV9KVe8/U
# 10/uQ2f9W4oP8t9UCBKWWupMAgZqNTB5f0gYEzIwMjYwODEyMjEyNzQyLjAyMlow
...
# IgQgA3c0XyrxvQWmsb6dfGYyD+s9nqEuWzCdLO4XXAydG1Mwgd0GCyqGSIb3DQEJ
...
# 8eu6iE+ys2Gla/OL9Xks4QhzNDANBgkqhkiG9w0BAQsFAASCAgAMS/oDMGERwxJZ
# lWFEhUdLLWNguKraxH0YcCcE+5vrUy9L277GV0NYY+9Fqyg1PSIns6dH9BA0Vi9R
# 7832Bh6qRQLur+VksDXrQLhDTnQW99geSua89/MlHAWhRVwE+WG8kFn+pXqWUErH
# H7VT06MwZWsxnvJcbNVdLAonmEZK3Dn+fKtKnpREzc2TvzAsMCkyZnmIHOuHSaSJ
# 1X2gGLqDIC89QGeujtMB2dRcAlUR1ahE9BOGB9JOhaSUum7qUIvkfww4QN0ysvuo
# EW1nylmulqd8235eUQOMUQ5AK1m1r5yarwgR5owbZE3WWLJQbW6TbG/d9ESn1Thz
# PzKM/tMPdNo3WIpnSIUWg2x9xLCT9rZ4jOKj6E3Ltgu7W57+s7GqmC9r1/F1nWOm
# o3xGTdmQ84aS926V20TRMQJv7bTC39cW9fIA5QGAMMSxdBW/rSctngr3ZJBsKv1Z
# S3Nvx610g2t7tzOvum5OoyhM7NLSiYlzMSEdOq8op5HfMX2oYgrENud8RrO6QVSk
# jHuL55cW6vmqP+gw9XDW+k4ASpbA5iZD67VzFHfn1HpyWnZ5CtsYjzI88atoLoPM
# lYbVF3G/5jEuqgScQuTDONr0sPaLAngQx2xNwchuj1iRa3Mikck44CYpyafiOnkA
# A4MdFRu/ccPAM7ClwZNP3aJUQYwgtA==
