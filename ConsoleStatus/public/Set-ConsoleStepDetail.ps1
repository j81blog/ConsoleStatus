function Set-ConsoleStepDetail {
    <#
        .SYNOPSIS
            Sets the text shown just before the status block of the current item.

        .DESCRIPTION
            Solves the case where the interesting value is only known once the work has run. Call
            it from inside an Invoke-ConsoleStep action, or between Write-ConsoleItem and
            Write-ConsoleResult. The detail is right aligned into the unused part of the progress
            field and truncated when it does not fit.

        .PARAMETER Text
            What the step found or did, kept short enough to sit next to the status. Whitespace is
            collapsed. An empty string clears a detail set earlier in the same item.

        .PARAMETER Append
            Keep what is already there and add this behind it, separated by '; '. A detail is short
            by nature, so this matters less than it does on a note, but the two behave the same.
            An empty string appends nothing rather than clearing.

        .EXAMPLE
            Set-ConsoleStepDetail -Text "$($users.Count) users"
    ...
    if (-not $Append) {
        $script:ConsoleStatus.Detail = $text
        return
    }
    ...
    $existing = $script:ConsoleStatus.Detail
    $script:ConsoleStatus.Detail = if ([string]::IsNullOrEmpty($existing)) { $text } else { "$existing; $text" }
}

# SIG # Begin signature block
# MII6AgYJKoZIhvcNAQcCoII58zCCOe8CAQExDzANBglghkgBZQMEAgEFADB5Bgor
# BgEEAYI3AgEEoGswaTA0BgorBgEEAYI3AgEeMCYCAwEAAAQQH8w7YFlLCE63JNLG
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCAt8gtZzz/kZrba
# UaC7A8pjEi/IewE89HSc6VBerJS8WaCCIiYwggXMMIIDtKADAgECAhBUmNLR1FsZ
...
# ADAZBgkqhkiG9w0BCQMxDAYKKwYBBAGCNwIBBDAvBgkqhkiG9w0BCQQxIgQgzyk7
# 9EB+hMussXIwrn9+H4lSq9yMP2PwQ22v/p6nsNUwDQYJKoZIhvcNAQEBBQAEggGA
# ayjqOBnH5ATqW/Y+ziT7AHdFWyvvUSeYzJjRj4V8yF5yIjfhyk0ohCHr6zfcGPAx
# 981xYqfNWgbQLj+S6u3LgCh8kp9ikI+btRJ/sIoUPYlhEOr4mVQCP4txVVYvDGGB
# 9gWnLDsboYL7TzyIEbfvwDaFonnfvxU1TLZTMoIMwN/FFyxc+5bM7fHYYKHNUb2r
# EV/k5HRtxPvzx+IYWSCT2kNzZXziqK25L32m468XbVjRyWxkemOXB2DTwIanin2r
# 4Evo+4ZHlf0iduYzbCI2vFkSSyFKL/W8ZzAN4UmkJYqpdr/DyyoXijafeLQdyaPx
# v1KomzQXUoETlRxCWU1l5Ncar4Csrw196lz69I+e9nOndsWpBQBLMumQncpJ2J8w
# lKfeevvX5vl9ao8u5ewdBz4m7d8tdvLrA6TrQCxptABsZIw1O79PcmVuOKrwsf1j
# pQjCfUJytdKtDy3xH7e0Mxx/2LzlBljEBnLLRrn5UJc+rpL4iwDn7ZAcgbKL4sHR
# oYIUsjCCFK4GCisGAQQBgjcDAwExghSeMIIUmgYJKoZIhvcNAQcCoIIUizCCFIcC
# AQMxDzANBglghkgBZQMEAgEFADCCAWoGCyqGSIb3DQEJEAEEoIIBWQSCAVUwggFR
# AgEBBgorBgEEAYRZCgMBMDEwDQYJYIZIAWUDBAIBBQAEIDbPcVjVEjLTyR0qVEK4
# WvMNg8OqvEXSLlhz4uoIAAYXAgZqNTB5f0YYEzIwMjYwODEyMjEyNzQwLjkwOFow
...
# IgQg9K/1WTQNhrvNxjXb/5XijjfZ9eLUnXAoAu0ydK/rTSowgd0GCyqGSIb3DQEJ
...
# 8eu6iE+ys2Gla/OL9Xks4QhzNDANBgkqhkiG9w0BAQsFAASCAgC7jQJy279MtfmY
# +eQlVaT12QydFMzGCtqD8fpnlwOKALcTIzU/pxbQ06nBpJgcych7YKdI9Kvu2lCC
# 9I2KCbev+ypOG69PLYkiWmzg0ajJkZo6iOwr55UGD/CFVmJoEAnxwTW7mGVVbtNw
# /vMV5j3fbZq0j+/37bp1bmHWztojZCT4BAXtmWawomOE+LyH63pd1qpOrQ/OMBud
# O1itAJ+zRNAR8BNl2uXytIDgqDxdQ0imJK8GybD0MZ/ehJjTKwG4RU1QMaY4r2DU
# UfcMQW8zMhaEF87At+drPh6awvRF+NJ5qYAi6bw6IftzpB8sLwEnY6Pcu/HmrddQ
# 2zO2ilQ8/MTXJWavEQRdKK//LFGkCBg4sQ161a+QbITzBUSysiypnBKn71RNgaxD
# lWhfvQG0QzaLpWqP+lQx9GbMtQLrT4IpAWvMEo+NNCH9X6/AeweXRybNUMNyc/N4
# 9Efn75DV5h65vJFb/VQygLOJaJlMVwZ+XV8x2tacge1PGeU5W/OYVqHx3Cdp4RsJ
# 2xTJ6XFccptqmUQqO7+mhMWEQNza6u5qirwpxzjuXjKtaCWdtRP1q2PbCiLVxhKs
# rFQgBH+rhoRbk5yY786Nt8Ce57Dc1cD+srSFp6ojKwRJ1RN8BIdAuqr45s5BNNk4
# acGbz8ntnu+wPsv7pwLhpOc2aDUHew==
