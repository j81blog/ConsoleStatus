function Write-ConsoleTick {
    <#
        .SYNOPSIS
            Writes one or more progress characters into the reserved field.

        .DESCRIPTION
            With a TotalSteps on the item, a tick is one unit of work and the bar is drawn
            proportionally, so the bar spans the field exactly however many units there are.

            Without it, a tick is one character and the bar continues on a new line when it runs
            out of cells. Continuation lines leave the label and value columns empty and are marked
            by BarContinuation, and only the last line of an item carries a status block.

        .PARAMETER Count
            How much progress to report in one call: units of work when the item declared a
            TotalSteps, characters otherwise. Defaults to 1.
    ...
    # A tick with no item open would draw loose glyphs onto whatever line the cursor is on, which
    # is what happens when a helper that ticks is also reachable outside a step.
    if (-not $cfg.ItemOpen -or $cfg.FieldWidth -lt 1) {
        return
    }

    # Known total: proportional bar.
    if ($cfg.TotalSteps -gt 0) {
        $cfg.DoneSteps += $Count

        if ($cfg.DoneSteps -gt $cfg.TotalSteps) {
            $cfg.DoneSteps = $cfg.TotalSteps
        }

        $target = [int][Math]::Floor(($cfg.DoneSteps / $cfg.TotalSteps) * $cfg.FieldWidth)
        $draw = $target - $cfg.Ticks

        if ($draw -le 0) {
            return
        }

        Write-HostText -Text ([string]$cfg.TickChar * $draw) -NoNewline -Color 'DarkCyan'
        $cfg.Ticks += $draw
        return
    }

    # Unknown total: wrap up to MaxBarLines, keeping the last cell of the final line for the marker.
    $left = $Count

    while ($left -gt 0) {
        $onLastLine = ($cfg.MaxBarLines -gt 0 -and $cfg.BarLines -ge $cfg.MaxBarLines)
        $limit = if ($onLastLine) { $cfg.FieldWidth - 1 } else { $cfg.FieldWidth }
        $remaining = $limit - $cfg.Ticks

        if ($remaining -le 0) {
            if (-not $onLastLine) {
                Start-ConsoleContinuation
                continue
            }

            if (-not $cfg.Overflowed) {
                $cfg.Overflowed = $true
                Write-HostText -Text ([string]$cfg.MoreChar) -NoNewline -Color 'DarkCyan'
                $cfg.Ticks++
            }
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCBFMAsO/TB5RVHM
# 3tjZwpiq7zJgFB01AAeczu485O6jSaCCIiYwggXMMIIDtKADAgECAhBUmNLR1FsZ
...
# ADAZBgkqhkiG9w0BCQMxDAYKKwYBBAGCNwIBBDAvBgkqhkiG9w0BCQQxIgQgoHHK
# DJP2GSAi7NNhm7C4ByfUR6mVWqpjr8c7s7SHrEswDQYJKoZIhvcNAQEBBQAEggGA
# MNuZ3EwNpIhcPchEuampCTZdFgdtkw5KKSfWXh+ctOmtjdlS39IjbvmKPEsMjwh6
# o7SY4eLLFilZPWs3E9kQG6Aa3G5Vcpz5e4HXgN3ZdlxzMhxTSZ6XUoi71TiSvaR9
# lWCgjsq4a3h5hUwLizX9AQwoxQw6SqLCeoqv1VtOy3bJJncbKuOzxXMqpppjmSG/
