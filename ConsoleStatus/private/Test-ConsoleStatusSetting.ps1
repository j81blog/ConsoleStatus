function Test-ConsoleStatusSetting {
    <#
        .SYNOPSIS
            Validates a single configuration key and value, and throws on anything unusable.

        .DESCRIPTION
            Unknown keys throw rather than being ignored, so a typo in $ConsoleStatusPreference is
            reported instead of silently doing nothing.

        .PARAMETER Name
            Setting name, which must be a key of $script:ConsoleStatusDefaults.

        .PARAMETER Value
            Proposed value, checked against the type and range for that setting.

        .PARAMETER Source
            Where the setting came from, quoted back in the exception so the author can find it.
    #>
    [CmdletBinding()]
    param(
    ...
    if (-not $script:ConsoleStatusDefaults.ContainsKey($Name)) {
        $valid = ($script:ConsoleStatusDefaults.Keys | Sort-Object) -join ', '
        throw "$Source contains unknown setting '$Name'. Valid settings are: $valid"
    }

    switch ($Name) {
        'Mode' {
            if ($Value -notin @('Column', 'Flow')) {
                throw "$Source : Mode must be 'Column' or 'Flow', got '$Value'."
            }
        }
        'BarContinuation' {
            if ($Value -notin @('Open', 'Blank')) {
                throw "$Source : BarContinuation must be 'Open' or 'Blank', got '$Value'."
            }
        }
        'LogAction' {
            if ($null -ne $Value -and $Value -isnot [scriptblock]) {
                throw "$Source : LogAction must be a scriptblock or `$null, got $($Value.GetType().Name)."
            }
        }
        { $_ -in @('Unicode', 'NoColor', 'ForceWidth', 'ShowDuration') } {
            if ($Value -isnot [bool] -and $Value -isnot [switch]) {
                throw "$Source : $Name must be a boolean, got '$Value'."
            }
        }
        { $_ -in @('TickChar', 'FillChar', 'RuleChar', 'MoreChar') } {
            if ([string]::IsNullOrEmpty([string]$Value) -or ([string]$Value).Length -ne 1) {
                throw "$Source : $Name must be exactly one character, got '$Value'."
            }
        }
        default {
            $ranges = @{
            ...
            if (-not [int]::TryParse([string]$Value, [ref]$number)) {
                throw "$Source : $Name must be a number, got '$Value'."
            }

            $range = $ranges[$Name]

            if ($number -lt $range[0] -or $number -gt $range[1]) {
                throw "$Source : $Name must be between $($range[0]) and $($range[1]), got $number."
            }

            if ($Name -eq 'ValueWidth' -and $number -gt 0 -and $number -lt 6) {
                throw "$Source : ValueWidth must be 0 (no value column) or at least 6, got $number."
            }

            if ($Name -eq 'Width' -and $number -gt 0 -and $number -lt 40) {
                throw "$Source : Width must be 0 (measure the console) or at least 40, got $number."
            }
        }
    }
}
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCDf08BXOJNq59kD
# 1fGMPM0pdFPjQR0abXregYGZQMxGJqCCIiYwggXMMIIDtKADAgECAhBUmNLR1FsZ
...
# ADAZBgkqhkiG9w0BCQMxDAYKKwYBBAGCNwIBBDAvBgkqhkiG9w0BCQQxIgQgbL23
# I0Kdd96BliXWzVRPcjSPGCcKoRUEPZdR6jwE9FEwDQYJKoZIhvcNAQEBBQAEggGA
# RmaDk3t9zc3zwvLqU9NITApNBPECRG0UefVSl+9L7spr16b8pDT75DMt00AzowBp
# 1SLKUs8UkH12Qa/z3yzgv5JlzI+LwwRw0Wd8oZ4JbiXGJABty2wOl+ll2E/NRs9L
# VwlMjpWwKZMh9nj5A/dZLRD4UQhwUkF4BhGUD2IIgGR3t5sezLq9F53xh4Ifd1FD
# G6KVbkkJKwQ1SsiZtuKkSJMufucovolQv2aUytQ7v+V8SZNh+T55qmHxyQTqztFr
# Vo2FMGU+BhuBQOwwsnEe8YIMTY4K0XXj5t2prqOk0EMI3DhiymxGqjFO7+hvHxpc
# /ARhDtzqS7ZxIk/K7gFg8zGoSYUBROe5Z3mb4eXvNPefPNt0HixUWCZBirbaKg+R
# I6u1VX4thNvt8AMTK9N8DuHOZodovEaIj3OyFLSsJxz/r6dmEXfWAt/UNaBoCB+B
# 294J2mHdx8+QH+JQ0N2HA7E53Gxckfd7E/fzZznGeMe5LFwL7ReyaMVN10JCwxY/
# oYIUsjCCFK4GCisGAQQBgjcDAwExghSeMIIUmgYJKoZIhvcNAQcCoIIUizCCFIcC
# AQMxDzANBglghkgBZQMEAgEFADCCAWoGCyqGSIb3DQEJEAEEoIIBWQSCAVUwggFR
# AgEBBgorBgEEAYRZCgMBMDEwDQYJYIZIAWUDBAIBBQAEIOEE9aF9G8MRAf6x84er
# yp9u++WImYALaGUo9wkH+XlgAgZqdgnSPswYEzIwMjYwODEwMjA0NzM4LjE2OVow
...
# IgQgnIZjDKSjLvltE2vwjig9GWjYms+TQF7pU3oOIu5UNu4wgd0GCyqGSIb3DQEJ
...
# PSanbLLmZ+tZpRgaUQwIiab9pTANBgkqhkiG9w0BAQsFAASCAgAxKsn0gKxP3Q2z
# vg8ePJcYRZXKqQGVz3YaW0y8oVEnoWjk9CeslCJ1QZSASap/DcFdjpFTSj+tKBkC
# YZ/3wmXeqsMRPINZhAC+UKuLnqJ6XYgd3le5E2hph/Q7Qnz3TatmY8q2b/vwMsJF
# ++AIIgYT3gn5E5RKSbt0kpasOol4KdMOaSxMl20zszaSVgSGSNRMUE/3PLPi5qiW
# zfoGVx1SkGF6AcLOEP+vp7Syrjq8ZAM0C++T8Rl5sDe+Hl2jYA3VI0PmGYraldnf
# BPK+mzqO/+XWvibzoGIg1+L/lq7uN598O4wwlkL+K90x6+veVO/kIx9IawItz2zt
# SuYeqMfsIMUti14uNl3o1A4WETok2Nzt9NFKK8FATzhMPaPv+3w+ZR53W3GL9MMv
# Bz6H9naoSp4WD9JswSytE/eLxEnwkBVc5wR0Q8YGTvIyq3MSpZ20TSzqBYEUQC94
# Lbny6ryLJ+rGXVFboZS1ECXStAaQPF1zvJ6oOMi3MtcSo6GwfQDJE2b66Gjm2YjU
# DW59TZvQEbIKifs5tzii5Xb2iLaxYrz1lEPZJh0D62nHvUC9Fa89G0WJvKvdP0YE
# ++XGHptusGzpkJ9uMSGW8zwS3F4bxSAwhwMHEKNiLvBlSrKSrAIClQHoz0wwchTl
# G48Oi4mbIRZA7uMzlwHAW7BbXdDqfQ==
