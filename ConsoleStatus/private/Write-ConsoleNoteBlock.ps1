function Write-ConsoleNoteBlock {
    <#
        .SYNOPSIS
            Writes one marked, indented, wrapped block of text underneath the current item.

        .DESCRIPTION
            Used for an overflowing detail, for an explicit note, and for the failure recap in the
            summary, so all three look the same.

        .PARAMETER Text
            The block to write. An empty string writes nothing.

        .PARAMETER Color
            Console colour for the text. The marker itself is always dim.
    #>
    [CmdletBinding()]
    param(
    ...
    $marker = (' ' * ($cfg.Indent + 2)) + [string]$cfg.MoreChar + ' '
    $blank = ' ' * $marker.Length
    $wrapWidth = $cfg.LineWidth - $marker.Length
    ...
    foreach ($chunk in (Split-TextToWidth -Text $Text -Width $wrapWidth)) {
        if ($first) {
            Write-HostText -Text $marker -NoNewline -Color 'DarkGray'
            $first = $false
        } else {
            Write-HostText -Text $blank -NoNewline
        }

        Write-HostText -Text $chunk -Color $Color
    }
}
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCBKXEV8z8vKVRRJ
# R/RqGz48TX6F+MyP3f562aon/Dz8WaCCIiYwggXMMIIDtKADAgECAhBUmNLR1FsZ
...
# ADAZBgkqhkiG9w0BCQMxDAYKKwYBBAGCNwIBBDAvBgkqhkiG9w0BCQQxIgQg8eG1
# Yu02DzSFn1G+Z5uB30Ms0tqZUhrH8OUmzLWxL6YwDQYJKoZIhvcNAQEBBQAEggGA
# jMk5hPxUBnNbSFqBDuq3n3PuxNQCPGc1l77rfD9GxQpl9v+CXD5699r8YmN1a+Vx
# ZonER3D9iwDDcvpAE42oUfDK5OCgoiCPhh58FzAr2f7WvTtE0feedb/JAZ8I93Qx
# y96NTENo3NBnXxMnQS/iPo2k3DHW6TgdJ3zuT4/IA+M6s77GL9blZriwS/ug/Wkk
# 9hsfX7QS6sVE5vWIhu7gtRWAkx+XfT8X0U0ZeOxlRLXRFh1Vfh+u3qRS/dEfifPl
# VHkB2UIl8576sCzKmm7WFsNshAq89qBqGXrduvaBLpAecH7Kt8QC5EV1YvmwT5hE
# SPUCSC2tbliPzJRcWkUvnI3n6trOXttaf94N8mhr15tz0QYzNsseh3ZgUlepavyZ
# JagXIsGHqNoMp9CZJ69YWnXT0NpDx07SIfPVQXyuBj/7Y9A6AaZO64jtmEpKygCB
# ZLmEVf/f76C22PO1/FsJwYEucydMDkaUhbFCdOoqdzA+MJNJt+mtllnWOPxMokvd
# oYIUsjCCFK4GCisGAQQBgjcDAwExghSeMIIUmgYJKoZIhvcNAQcCoIIUizCCFIcC
# AQMxDzANBglghkgBZQMEAgEFADCCAWoGCyqGSIb3DQEJEAEEoIIBWQSCAVUwggFR
# AgEBBgorBgEEAYRZCgMBMDEwDQYJYIZIAWUDBAIBBQAEICQ49tL07+Mydawgjr96
# XzDlx2cTXwsFvT9HJoMUMVtQAgZqNTBzBWQYEzIwMjYwODEwMjA0NzQwLjIyOFow
...
# IgQglKtMREAdq7crJt7SWqv5SPtMh63VIHDrS+diFSfCp+swgd0GCyqGSIb3DQEJ
...
# r2m/ztk0VK82Yn22ky/ozUOgUzANBgkqhkiG9w0BAQsFAASCAgDevaCuk5ySY7gA
# jedG06UzaL/Ol7cO5F/2inKrr5ErLgXtA01rPhiT7u5cC/0P+5mmWqYi9chk9q5F
# ud/jDa5pyATHG4sXalavmcSQ+bfM53l469GFPTvPrz/iZuX9/SfLEN3AR8QIqwrz
# chrx3nj9S5WrH4ReOhcM6A2TqQsr6BD8TRDmN3kLJwVlgZMPRAPB6Lo6UjpIhOvX
# 7rketGgw/LRFgjyCM44w0MuYc+cB49uzX59iZ6PlLChvWLbzysZEMVBUMEDqP4Rx
# REwSX4gNwA38DM8bfvXeGvdyQkZvW/0wXEt/M086MZFm++8HarUVLoxbk5VVi19p
# 4c29SZYskDQWmplyaxOuXCdobr6OraWHQJd9HnQrE4utRy6VB1sk0+CrQ91ZtmXG
# ypfUcBhfECifp1s3SbMPPDTJzecO68avjae1diZYyZWUaAGKaBWl+opo/S0R9U0o
# pbpWtAGFKI1BUqCZ+kZRxP5Zn1tYMHDUWktkhTh7oo7z9bLwB3m+e23oUQhGALXo
# SjJyu1FS4i4bmK9WO71uQ2HaEVPtqaMGm50DNDUEaEyU4Jt76DBAgjUPxdfohiKm
# Y8nX2JwdwcQ8Io2xJebeBt9K1s3b+aXtrj6HHNkcskHTD5XdaHplFTeMM2Yp4Jj0
# /RbbVMlp8G3+h2JrjmeomDP37Xwpng==
